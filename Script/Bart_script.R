...
# Retrain the model
sdm <- bart(x.train=Present_all.cov[, step.model], y.train=Present_all.cov[, 'Sapidus_Pres'],
            keeptrees = TRUE)
summary(sdm)
#--------------------------- Retune: cross-validation
RET_sdm<-retune(sdm, reps = 10) 
summary(RET_sdm)

#---------- Present
map_present <- predict(RET_sdm, Present_covs, quantiles=c(0.025, 0.975),quiet=TRUE)
plot(predict(RET_sdm, Present_covs, quiet=TRUE))

plot(map_present, main='Predicted probability',col=cl, 
     box=F, axes=F)
par(mfrow=c(2,2))
par(mar=c(2,1,2,5))
plot(map_present[[1]], 'Posterior mean',col=cl,  
     box=F, axes=F)
plot(map_present[[2]], 'Lower 95% CI bound', col=cl, 
     box=F, axes=F)
plot(map_present[[3]], 'Upper 95% CI bound',col=cl,  
     box=F, axes=F)
plot(map_present[[3]]-map_present[[2]], 'Credible interval width', col=cl, 
     box=F, axes=F)
...
par(mfrow=c(2,2))
par(mar=c(2,1,2,5))
plot(map_RCP45_2050[[1]], 'Posterior mean',col=cl,  
     box=F, axes=F)
plot(map_RCP45_2050[[2]], 'Lower 95% CI bound', col=cl, 
     box=F, axes=F)
plot(map_RCP45_2050[[3]], 'Upper 95% CI bound',col=cl,  
     box=F, axes=F)
plot(map_RCP45_2050[[3]]-map_RCP45_2050[[2]], 'Credible interval width', col=cl, 
     box=F, axes=F)

#---------- RCP45_2100_covs
names(RCP45_2100_covs)
names(RCP45_2100_covs) <- c('Pres_Cur', 'Pres_Sal', 'Pres_Temp')# rename rasters
map_45.2100 <- predict(RET_sdm, RCP45_2100_covs,quantiles=c(0.025, 0.975), quiet=TRUE)

par(mfrow=c(2,2))
par(mar=c(2,1,2,5))
plot(map_45.2100[[1]], 'Posterior mean',col=cl,  
     box=F, axes=F)
plot(map_45.2100[[2]], 'Lower 95% CI bound', col=cl, 
     box=F, axes=F)
plot(map_45.2100[[3]], 'Upper 95% CI bound',col=cl,  
     box=F, axes=F)
plot(map_45.2100[[3]]-map_45.2100[[2]], 'Credible interval width', col=cl, 
     box=F, axes=F)
...
plot(predict(RET_sdm, map_RCP85_2050, quiet=TRUE))

par(mfrow=c(2,2))
par(mar=c(2,1,2,5))
plot(map_RCP85_2050[[1]], 'Posterior mean',col=cl,  
     box=F, axes=F)
plot(map_RCP85_2050[[2]], 'Lower 95% CI bound', col=cl, 
     box=F, axes=F)
plot(map_RCP85_2050[[3]], 'Upper 95% CI bound',col=cl,  
     box=F, axes=F)
plot(map_RCP85_2050[[3]]-map_RCP85_2050[[2]], 'Credible interval width', col=cl, 
     box=F, axes=F)
...
map_85_2100 <- predict(RET_sdm, RCP85_2100_covs,quantiles=c(0.025, 0.975), quiet=TRUE)
plot(predict(RET_sdm, map_85.2100, quiet=TRUE))

par(mfrow=c(2,2))
par(mar=c(2,1,2,5))
plot(map_85_2100[[1]], 'Posterior mean',col=cl,  
     box=F, axes=F)
plot(map_85_2100[[2]], 'Lower 95% CI bound', col=cl, 
     box=F, axes=F)
plot(map_85_2100[[3]], 'Upper 95% CI bound',col=cl,  
     box=F, axes=F)
plot(map_85_2100[[3]]-map_85_2100[[2]], 'Credible interval width', col=cl, 
     box=F, axes=F)

Present_sp.Temp <- spartial(RET_sdm, Present_covs, 
...
plot(map_RCP45_2050[[1]]-map_present[[1]], 'Gain and Loss-45.2050', col=cl, 
     box=F, axes=F)
plot(map_RCP45_2050[[1]]-map_present[[1]], 'Gain and Loss-45.2100', col=cl, 
     box=F, axes=F)
...
#---------------------------  Covariate Plots
plot(stack(RCP85_2100_covs),col=cl) #cropped files

save(RCP85_2100_covs, 
     file='~/efc/R_Stat/2022_NEMA/SDM_Bayes/5_R_files/RCP85_2100_Covs.rds')
load('~/efc/R_Stat/2022_NEMA/SDM_Bayes/5_R_files/RCP85_2100_Covs.rds') 

#---------------------------  Rasterizations and point by grid cell
Sapidus <- SpatialPointsDataFrame(Sapidus[, 2:3], data.frame(RCP85_2100 = Sapidus[, 1]))
tmp <- rasterize(Sapidus, RCP85_2100_covs[[1]], field = "RCP85_2100", fun = "min")
pts.sp1 <- rasterToPoints(tmp,
                          fun = function(x) {
                            x > 0
                          })
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#                               Extract RCP85_2100 values                        #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Extract RCP85_2100ence values
RCP85_2100.cov <- raster::extract(RCP85_2100_covs, pts.sp1[, 1:2])
RCP85_2100.cov <- na.omit(RCP85_2100.cov)
head(RCP85_2100.cov) #602
nrow(RCP85_2100.cov)
nrow(pts.sp1)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Loading env variables
absence <- randomPoints(RCP85_2100_covs, nrow(RCP85_2100.cov))
abs.cov <- raster::extract(RCP85_2100_covs, absence)
plot(absence)
#--------------------------- Code the response
RCP85_2100.cov <- data.frame(RCP85_2100.cov)
RCP85_2100.cov$Sapidus <- 1
abs.cov <- data.frame(abs.cov)
abs.cov$Sapidus <- 0

#--------------------------- And one to bind them
RCP85_2100_all.cov <- rbind(RCP85_2100.cov, abs.cov)
RCP85_2100_all.cov <- RCP85_2100_all.cov[complete.cases(RCP85_2100_all.cov), ]
nrow(RCP85_2100_all.cov)
head(RCP85_2100_all.cov)

RCP85_2100_xvars <- names(RCP85_2100_all.cov)[!(names(RCP85_2100_all.cov) == 'Sapidus')]
RCP85_2100_xvars
...
#                          Correlation and collinearity                        #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
RCP85_2100_vif<-vifstep(RCP85_2100_all.cov[,-5])
RCP85_2100_vif

library(GGally)
ggpairs(RCP85_2100_all.cov[,-5])

library(ppcor)
pcor(RCP85_2100_all.cov[,1:3], method = "pearson")

corrplot.mixed(RCP85_2100_all.cov[,1:3], lower.col = 'black', number.cex = .7)

tiff(file="~/efc/R_Stat/2022_NEMA/SDM_Bayes/3_Plots/RCP85_2100_all.cov.tiff",
     width=6, height=4, units="in", res=600)

corrplot(cor(RCP85_2100_all.cov[,-5]),addCoef.col = 1,number.cex = 0.5,
         order = 'AOE',type = 'upper') 
dev.off()
