...
#Cuidado
save.image("CaliBART.RData")

setwd("~/efc/R_Stat/2022_NEMA/SDM_Bayes/1_Species")
load(file = "CaliBART.RData") # save work space


#--------------------------- Labraries/ packages 
packages = c('rgdal','spdep', 'tmap','tidyverse','dismo','maptools','sf','report', 
             'performance','sp','rgeos','ggplot2','plotly','raster','tidyr',
             'dplyr','sdm', 'rworldmap', 'tmap','usdm', 'mapview', 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
EEZ <-read_sf(
  '~/efc/R_Stat/2022_NEMA/SDM_Bayes/2_Predictors/EEZ_shapes/EEZ_NoISla.shp'
)
plot(EEZ,col='aliceblue')

writeOGR(EEZ, driver="ESRI Shapefile",
         dsn = "C:/2022_QGIS/RCP_Callinectes" , layer = "EEZFinal")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Present_vif<-vifstep(Present_all.cov[,-7])
Present_vif

library(GGally)
ggpairs(Present_all.cov[,-7])

library(ppcor)
pcor(Present_all.cov[,-7], method = "pearson")
...
partialEU(RET_Present.model,
          c('Pres_Temp','Pres_Sal','Pres_pH','Pres_PrimProd','Pres_Cur','Pres_Rugosity'),
          trace = FALSE,
          ci = TRUE,
          panel = TRUE,
          smooth = 8)
...
library(ppcor)
pcor(RCP45_2050_all.cov[,-5], method = "pearson")

corrplot.mixed(RCP45_2050_all.cov[,-5], lower.col = 'black', number.cex = .7)
...
load('~/efc/R_Stat/2022_NEMA/SDM_Bayes/5_R_files/RCP45_2050_CV.model.rds') 
...
library(ppcor)
pcor(RCP45_2100_all.cov[,-5], method = "pearson")
...
corrplot(cor(RCP45_2100_all.cov[,-5]),addCoef.col = 1,number.cex = 0.5,
         order = 'AOE',type = 'upper') 
dev.off()
...
load('~/efc/R_Stat/2022_NEMA/SDM_Bayes/5_R_files/RCP45_2100_CV.model.rds') 
...
corrplot(cor(RCP85_2050_all.cov[,-5]),addCoef.col = 1,number.cex = 0.5,
         order = 'AOE',type = 'upper') 
dev.off()
...
load('~/efc/R_Stat/2022_NEMA/SDM_Bayes/5_R_files/RCP85_2050_CV.model.rds') 
...
#---------------------------  Rasterizations and point by grid cell
Sapidusdf<-as.data.frame(Sapidus)
nrow(Sapidus)
Sapidus <- SpatialPointsDataFrame(Sapidus[, 1:2], data.frame(Presence = Sapidus[, 3]))
tmp <- rasterize(Sapidus, RCP85_2100_covs[[1]], field = "Presence", fun = "min")
pts.sp1 <- rasterToPoints(tmp,
                          fun = function(x) {
                            x > 0
                          })
...
library(ppcor)
pcor(RCP85_2100_all.cov[,-5], method = "pearson")

corrplot.mixed(RCP85_2100_all.cov[,-5], lower.col = 'black', number.cex = .7)

tiff(file="~/efc/R_Stat/2022_NEMA/SDM_Bayes/3_Plots/RCP85_2100_all.cov1.tiff",
     width=10, height=4, units="in", res=600)

corrplot(cor(RCP85_2100_all.cov[,-5]),addCoef.col = 1,number.cex = 0.8,
         order = 'AOE',type = 'upper') 
dev.off()
...
load('~/efc/R_Stat/2022_NEMA/SDM_Bayes/5_R_files/RCP85_2100_CV.model.rds') 
